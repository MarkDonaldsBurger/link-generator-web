    ...
    if token:
        ...
        with c1:
            ...
            if st.button("Generate Internal"):
                ...
                sheet.append_row([status, t_no, "INTERNAL", datetime.now().strftime("%Y-%m-%d"), token, "https://forms.office.com/r/5s3GA7Df0T"])
                st.success("Internal link generated!")
                st.rerun()
            if st.button("Generate External"):
                token = str(uuid.uuid4())
                sheet.append_row([status, t_no, "EXTERNAL", datetime.now().strftime("%Y-%m-%d"), token, "https://forms.office.com/r/KchEak7FWA"])
                st.success("External link generated!")
                st.rerun()
        ...
        with c2:
            ...
            with u4:
                st.write("")
                if st.button("Update"):
                    match = df_tokens[(df_tokens["Ticket No."] == sel) & (df_tokens["Type"] == link_type)]
                    if not match.empty:
